...
class EventDelLandingProcessor(BaseLandingProcessor):
    def __init__(self, spark, metadata_df):
        super().__init__(spark, metadata_df, "event_del")
        self.key_columns = ["event_id"]  # Replace with actual key columns
        self.non_null_columns = ["event_id", "deletion_date"]  # Replace with actual non-null columns
...
def main():
    try:
        ...
        spark = SparkSession.builder \
            .appName("Event_Del_Landing_Processor") \
        ...
        # Process event deletes
        processor = EventDelLandingProcessor(spark, metadata_df)
        processor.process()
