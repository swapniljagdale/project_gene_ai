...
class ChildAttributesLandingProcessor(BaseLandingProcessor):
    def __init__(self, spark, metadata_df):
        super().__init__(spark, metadata_df, "lm_child_attributes")
        self.key_columns = ["attribute_id"]  # Replace with actual key columns
        self.non_null_columns = ["attribute_id", "hcp_id", "attribute_name", "attribute_value"]  # Replace with actual non-null columns
        
    def validate_integrity(self, df, hcp_master_df):
        """Validate referential integrity with HCP master."""
        try:
            # Check for child attributes with no matching HCP in master
            integrity_issues = df.join(
                hcp_master_df.select("hcp_id"),
                on="hcp_id",
                how="left_anti"
            )
            ...
            # Get clean records
            clean_df = df.join(
                integrity_issues.select("attribute_id"),
                on="attribute_id",
                how="left_anti"
            )
    ...
    def process(self):
        try:
            ...
            # Read HCP master data for integrity check
            hcp_master_df = self.spark.read.parquet(f"{self.landing_path}lm_hcp_master")
            ...
            # Validate referential integrity with HCP master
            clean_df, integrity_issues = self.validate_integrity(clean_df, hcp_master_df)
            if integrity_issues and not integrity_issues.rdd.isEmpty():
                self.write_to_error(integrity_issues, "Referential integrity issue - HCP not found in master")
...
def main():
    try:
        ...
        spark = SparkSession.builder \
            .appName("Child_Attributes_Landing_Processor") \
        ...
        processor = ChildAttributesLandingProcessor(spark, metadata_df)
        processor.process()
