...
class EventLandingProcessor(BaseLandingProcessor):
    def __init__(self, spark, metadata_df):
        super().__init__(spark, metadata_df, "event")
        self.key_columns = ["event_id"]  # Replace with actual key columns
        self.non_null_columns = ["event_id", "event_date"]  # Replace with actual non-null columns
...
def main():
    try:
        ...
        spark = SparkSession.builder \
            .appName("Event_Landing_Processor") \
        ...
        # Process events
        processor = EventLandingProcessor(spark, metadata_df)
        processor.process()
