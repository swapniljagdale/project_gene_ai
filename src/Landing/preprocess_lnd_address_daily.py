...
class AddressLandingProcessor(BaseLandingProcessor):
    def __init__(self, spark, metadata_df):
        super().__init__(spark, metadata_df, "lm_address")
        self.key_columns = ["address_id"]  # Replace with actual key columns
        self.non_null_columns = ["address_id", "hcp_id", "address_line1", "city", "state", "zip"]  # Replace with actual non-null columns
        
    def validate_integrity(self, df, hcp_master_df):
        """Validate referential integrity with HCP master."""
        try:
            # Check for addresses with no matching HCP in master
            ...
            # Get clean records
            clean_df = df.join(
                integrity_issues.select("address_id"),
                on="address_id",
                how="left_anti"
            )
...
def main():
    try:
        ...
        spark = SparkSession.builder \
            .appName("Address_Landing_Processor") \ 
        ...
        processor = AddressLandingProcessor(spark, metadata_df)
        processor.process()
